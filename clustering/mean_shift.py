...
class MeanShift(object):
    ...
    def fit(self, points, kernel_bandwidth):
        ...
        while True:
            ...
            for i in range(0, len(shift_points)):
                ...
                shift_points[i] = self._shift_circle(shift_points[i], points, 1.0)
    ...
    def _shift_circle(self, point, points, r):
        #import ipdb;ipdb.set_trace()
        shift_x = 0.0
        shift_y = 0.0
        scale = 0.0
        ...
        for i, point in enumerate(points):
            if(len(cluster_ids) == 0):
                ...
                for center in cluster_centers:
                    ...
                    if(dist < CLUSTER_THRESHOLD): #same center as in the current center list
                        cluster_ids.append(cluster_centers.index(center))
                if(len(cluster_ids) < i + 1): #new center appended
                    cluster_ids.append(cluster_idx)
                    cluster_centers.append(point)
                    cluster_idx += 1
        return cluster_ids
